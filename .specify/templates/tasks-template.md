...
**Tests**: Include tests proportional to risk. Workspace authorization, schema ownership,
cross-boundary contracts, AI approvals, and critical revenue flows require automated coverage.
...
- **Web app**: `backend-ai/app/`, `frontend/src/`
...
- [ ] T010 Add workspace authorization and data-isolation tests
- [ ] T011 Document schema ownership and typed cross-boundary contracts
...
### Tests for User Story 1 (include when required by risk or explicitly requested) ⚠️
...
### Tests for User Story 2 (include when required by risk or explicitly requested) ⚠️
...
### Tests for User Story 3 (include when required by risk or explicitly requested) ⚠️
...
- [ ] TXXX Audit AI approvals, destructive actions, and recovery behavior
- [ ] TXXX Review workspace isolation across reads, writes, summaries, and AI requests
- [ ] TXXX Remove accidental placeholder metrics and later-roadmap scope expansion
...
- Add tests proportional to risk and verify they fail before implementing
- Treat missing workspace scope as a release-blocking defect
- Require explicit human approval for external and destructive AI actions
- Keep `frontend/` product-data migrations separate from `backend-ai/` `ai_*` migrations
