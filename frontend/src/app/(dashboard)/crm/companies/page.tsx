...
export default async function CompaniesPage() {
  ...
  return (
    <div className="grid gap-5">
      ...
      {canWrite ? (
        <Panel>
          <h2 className="font-display text-2xl font-bold tracking-wide">Add company</h2>
          <form action={createCompanyAction} className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Select name="status" defaultValue="prospect" options={[{ value: "prospect", label: "Prospect" }, { value: "active", label: "Active" }, { value: "customer", label: "Customer" }, { value: "inactive", label: "Inactive" }]} />
          ...
          {records.map((record) => (
            <RecordCard key={record.id} title={record.name} metadata={record.industry || "Industry not set"}>
              <div className="flex items-center justify-between gap-3">
                ...
                <Building2 className="size-4 text-[var(--dashboard-icon)]" />
              </div>
              <p className="mt-3 text-sm text-[var(--dashboard-muted)]">{record.website || record.email || "No contact details yet."}</p>
            </RecordCard>
          ))}
        </section>
      ) : (
