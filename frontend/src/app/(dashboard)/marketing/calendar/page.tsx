...
export default async function ContentCalendarPage() {
  ...
  return <div className="grid gap-5"><PageHeader kicker="Content calendar" title="See planned campaign touchpoints in one timeline." description="Calendar items are internal plans for manual review and publishing." />
    <Panel><h2 className="flex items-center gap-2 font-display text-2xl font-bold tracking-wide"><CalendarDays className="size-5 text-[var(--dashboard-accent)]" /> Upcoming schedule</h2>
      <div className="mt-4 grid gap-2">{items.map((item) => <article key={`${item.kind}-${item.id}`} className="flex flex-col justify-between gap-3 rounded-xl border border-[var(--dashboard-border)] bg-[var(--dashboard-control)] shadow-sm p-3 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--dashboard-icon)]">{item.kind}</p><p className="mt-1 text-sm font-bold">{item.title}</p></div><div className="flex items-center gap-3"><StatusBadge value={item.status} /><p className="text-xs text-[var(--dashboard-muted)]">{formatDate(item.at)}</p></div></article>)}{items.length === 0 ? <EmptyState title="Nothing scheduled" description="Scheduled social and email drafts will appear here." /> : null}</div>
    </Panel></div>;
}
