...
export function ContactTimeline({ contactId, activity }: { contactId: string; activity: Activity[] }) {
  ...
  return (
    <div className="grid gap-5">
      <form action={addNote} className="flex gap-2">
        ...
        <Button className="rounded-full bg-[#d8ff62] px-4 font-bold text-[#10211c] hover:bg-[#e5ff92]">Add note</Button>
      </form>
        ...
        {activity.map((entry) => (
          <article key={entry.id} className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-3">
            ...
            <p className="mt-1 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[#71817b]">
              <Clock3 className="size-3" />{entry.action.replace(".", " · ")} · {entry.createdAt.toLocaleString()}
            </p>
          </article>
        ))}
      </div>
      <form action={remove} className="rounded-xl border border-red-300/15 bg-red-300/5 p-3">
        ...
        <Button variant="destructive" className="mt-3 rounded-full"><Trash2 />Remove contact</Button>
      </form>
    </div>
  );
}
